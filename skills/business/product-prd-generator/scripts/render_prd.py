...
STATUS_ORDER = ("existing", "partial", "missing", "explicitly-not-do")
VERSION_BY_STATUS = {
    "existing": "P3",
    "partial": "P1",
    "missing": "P2",
...
def _load_inputs(reconcile_path: str, doc_map_path: str | None) -> RenderInputs:
    ...
    return RenderInputs(reconcile=reconcile, doc_map=doc_map)
...
def _confidence_stats(capabilities: list[dict[str, Any]]) -> Counter[str]:  # noqa: ANY_OK
    return Counter(cap.get("confidence", "low") for cap in capabilities)
...
def _render_version_plan(capabilities: list[dict[str, Any]]) -> str:  # noqa: ANY_OK
    ...
    for cap in capabilities:
        status = cap.get("reconciled_status", "missing")
        if status == "existing" and not cap.get("gaps"):
            continue
