...
compatibility: >
  ...
  能力对齐参考 product-prd-generator 输出的功能清单与 ontology（$LANLNK_BASE/knowledge/business-ontology.yaml）。
  商管域知识参考 $LANLNK_BASE/prd/商管系统/域知识.md。
...
竞品原始/精华资料继续走现有 `materials/13-competitors/<vendor>/` 六类分类：

| 分类 | 在本 skill 中的证据强度 |
|---|---|
| `04-产品功能/` | 强证据（功能清单、数据字典、业务逻辑）|
| `05-实施与服务/` | 强证据（操作手册、功能手册、蓝图方案）|
| `02-方案汇报/` | 弱证据（汇报会夸大，需运行时或手册佐证）|
| `01-销售策略/` | 极弱证据（销售话术，不作产品事实）|
| `03-报价商务/` | 仅作定价参考，不进能力矩阵 |
| `06-行业洞察/` | 仅作背景，不进能力矩阵 |

转换中间产物（可重生）继续走 `$LANLNK_BASE/raw/prd-商管系统/02-competitors/<vendor>/`，已被 `.gitignore` 排除。

### 输出（新建在 PRD 项目下）

```text
$LANLNK_BASE/prd/商管系统/competitor-analysis/<vendor>/
├── .auth.json                          ← demo 凭据（mode 0600，gitignored）
├── source-inventory.json               ← 资料盘点（每份资料的分类/日期/可信度）
├── source-inventory.md
├── runtime-probe.json                  ← demo 探测结果（菜单/页面/流程/截图清单）
├── runtime-probe.md
├── imgs/                               ← demo 截图（已脱敏）
├── capability-map.json                 ← 竞品能力结构化清单
...
> **为什么不放回 materials/13-competitors**：materials 是「竞品原始资料库」，本 skill 输出是「蓝联对竞品的分析结论」，性质不同。放在 PRD 项目下便于 product-prd-generator / strategy-brief-generator 复用，且与现有 `prd/商管系统/output/` 同根。

## 处理流程

```
P0: 输入解析 → 判定模式（manual-only / demo-only / manual-plus-demo）+ 资料盘点
    ↓
P1: 文档转换（manual 模式，复用 material-importer）
    ↓ raw/*.md + raw/*_media/
P2: Demo 运行时探测（demo 模式，复用 doc-generator 模式 + Playwright）
    ↓ runtime-probe.json + imgs/
P3: 能力抽取（统一 schema，从文档 + 运行时合并）
    ↓ capability-map.json
P4: 术语归一（对齐商管域知识 + ontology）
    ↓
P5: 蓝联现状映射（读 product-prd-generator 的功能清单）
    ↓
P6: 加权对比矩阵 + 改进建议分级
    ↓ ability-comparison-matrix.json + improvement-recommendations.md
P7: 证据台账 + 待确认项
    ↓ evidence-ledger.json + review/pending-items.md
P8: 交互确认与交付
```
...
|---|---|---|
| 竞品名 | 用于目录命名 | 旗茂 / 海鼎 / 明源 |
| 资料来源 | manual 路径 / demo URL / 两者 | `materials/13-competitors/qimao/` + `http://oa.1qmall.cn/SMallTest/` |
| demo 凭据（如有）| 用户名/密码 | llkj / llkj123 |
| 对照产品 | 蓝联哪个产品作基准 | MI（商管系统）/ CRM / AI |
...
输出 `source-inventory.json`：每份资料记录 `路径 / 分类 / 文件日期 / 转换状态 / 可信度`。

### P1 文档转换（manual 模式）

复用 `material-importer` 的 markitdown + 图片提取 + OCR 能力：

```bash
cd /opt/code/skill/skills/business/material-importer
uv run scripts/extract_images.py "<竞品资料目录>"
markitdown "<原始文件>" -o "<raw 目录>/<同名>.md"
```

转换规则与 material-importer 一致：PPTX/DOCX/XLSX/PDF → markdown，图片提取到 `raw/<filename>_media/`，空章节清理。

**OCR**：图片型资料（扫描手册、PPT 截图）用 DeepSeek-OCR-2，复用 material-importer 的 `scripts/ocr_extract.py`。

**不做**：不把转换结果直接写入 `materials/13-competitors/`，只写 `raw/prd-商管系统/02-competitors/<vendor>/`。

### P2 Demo 运行时探测（demo 模式）

**复用 `doc-generator` 的运行时探测与截图规范**，但产物落到本 skill 的输出目录。详细页面抽取策略见 `references/page-extraction-strategy.md`。

**P2 执行顺序**：

1. **凭据写入** `.auth.json`（mode 0600），首次询问用户，二次复用
...
7. **页面字段抽取**（三档降级）：
   - **P2.4 模式 A（同源 fetch + 正则）**：默认起点，一次 evaluate 批量抓 10-20 页，每页 50ms。HTML 含字段名/按钮文本的 layui/服务器渲染站点最有效
   - **P2.5 模式 B（addTab + iframe.contentDocument）**：模式 A 失败（fields=[] 或全是噪音）时升级，每页 3-5s。⚠️ **禁止在 iframe 内调用 element.click()**——会触发跳转导致 page URL 重置为 about:blank，会话失效
   - **P2.6 模式 C（Playwright snapshot）**：兜底，每页 5-10s，仅用于 P0 必补的重点页面
8. **流程推断**：从按钮组合识别 CRUD/查询/导出/审批等典型流程
9. **截图**：每模块选 1-3 张关键页面（首页 + 重点表单/报表），存到 `imgs/`
10. **locator 规范**：只用 text/role/placeholder/label，禁用 CSS/XPath（与 doc-generator 一致）
11. **单页失败容忍**：一个页面 timeout 或 fetch 失败不阻塞整体，记入 manifest
12. **权限标记**：demo 账号无法访问的页面标 `accessible: false` + `requires_role`，**不判定为竞品没有此功能**
13. **写 runtime-probe.json**：菜单/路由/元素/流程/截图清单 + 探测时间 + 账号权限等级 + 合规状态（captcha_observed / tos_conflict_detected）
...
从文档（P1）和运行时（P2）合并抽取能力，统一 schema（详见 `references/capability-schema.md`）：
...
- `$LANLNK_BASE/prd/商管系统/域知识.md`（商管域术语别名表）
- `$LANLNK_BASE/knowledge/business-ontology.yaml`（8 模块 482 术语）
...
| 对照产品 | 功能清单路径 |
|---|---|
| MI / 商管系统 | `$LANLNK_BASE/prd/商管系统/output/功能清单.md` |
...
3. **`$LANLNK_BASE/knowledge/sales/methodology/15-商业地产岗位病药矩阵.md`**（多岗位共同痛点加权）——长期参考
4. **`$LANLNK_BASE/prd/<产品>/output/功能清单.md` 的 missing/partial 项分布**——反向推断客户痛点（missing 多的维度 = 蓝联弱项 = 应加权）
...
```
Agent:
  [P0] 模式: manual-plus-demo | 竞品: qimao | 对照产品: MI
       资料盘点: 12 份手册（操作手册 5 + 功能手册 4 + 数据字典 1 + 蓝图 2）
       demo: http://oa.1qmall.cn/SMallTest/ | 账号角色待探测

  [P1] 转换 12 份文档 → raw/prd-商管系统/02-competitors/qimao/（12 md + 138 张图）
       OCR: 数据字典 PDF（扫描版）→ DeepSeek-OCR-2

  [P2] Playwright 探测: 登录 → 菜单 8 项 → 逐页探测 → 截图 47 张
       权限: llkj 为管理员账号，可访问全部模块
       写 .auth.json（mode 0600）+ runtime-probe.json

  [P3] 抽取能力 86 项（功能 52 + 流程 18 + 字段 12 + 权限 4）
  [P4] 术语归一: 79 项匹配 ontology，7 项进 review
  [P5] 蓝联现状映射: existing 41 / partial 23 / missing 18 / better-than-lanlnk 4
  [P6] 加权矩阵 → 改进建议: 补齐 12 / 增强 19 / 借鉴 8 / 观察 47
  [P7] 证据台账 142 条 | high 68% / medium 24% / low 8%
       review: 7 项术语未映射 + 2 项 demo 权限未知 + 3 项手册版本过旧

  [P8] 交付: competitor-analysis/qimao/
...
```
Agent:
  [P0] 模式: demo-only | 竞品: xx | 对照产品: MI
       警告: 无手册佐证，能力清单置信度上限 medium，建议补充手册

  [P2] Playwright 探测 → runtime-probe.json + imgs/
  [P3] 抽取能力（仅运行时可见部分）
  [P5] 蓝联现状映射（仅对运行时可见能力）
  [P6] 改进建议（标注"仅基于运行时探测，建议补充手册复核"）
  [P8] 交付 + 提示: manual 模式补完后可重跑 manual-plus-demo 提升置信度
```
...
| 变量 | 路径 | 说明 |
|---|---|---|
| `$MATERIALS_DIR` | `$LANLNK_BASE/materials` | 竞品原始资料根 |
| `$COMPETITORS_DIR` | `$MATERIALS_DIR/13-competitors` | 竞品库（6 类分类）|
| `$RAW_DIR` | `$LANLNK_BASE/raw/prd-商管系统/02-competitors/<vendor>/` | 转换中间产物（gitignored）|
| `$PRD_DIR` | `$LANLNK_BASE/prd/商管系统` | PRD 项目根 |
| `$ANALYSIS_ROOT` | `$PRD_DIR/competitor-analysis/<vendor>` | 本 skill 输出根 |
| `$USERGUIDE_BASE` | `$LANLNK_BASE/UserGuide` | doc-generator 的输出根（如需生成竞品操作手册）|
...
6. **新增竞品**时，在 `materials/13-competitors/` 建对应子目录（6 类分类），并在本 skill 输出目录建 `competitor-analysis/<vendor>/`
