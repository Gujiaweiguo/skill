...
- **Fresh-clone 部署**（跨 skill，治理动作）：`.opencode/` 在根 `.gitignore` 第 14 行（运行时状态，不入版本控制），所以 `git clone` 后所有 skill 软链都缺失，`check_skill_ecosystem.sh` 会出 21 条 P0 FAIL。**首次部署 workaround**：跑一次 `references/scripts/setup_skill_symlinks.sh`，幂等创建所有 21 个软链（已存在的正确软链 SKIP，错链 REPLACE，断链留给人工检查）。然后跑 `check_skill_ecosystem.sh` 验证全绿。
- **Pre-commit hook**（跨 skill，治理动作）：`references/scripts/pre-commit.sh` 是 tracked 脚本，在 commit 触及 `skills/**/SKILL.md` / `.opencode/skills/` / `AGENTS.md` / `.gitignore` 时自动跑 `check_skill_ecosystem.sh --ci`，失败则阻止 commit。`.git/hooks/` 不入版本控制，需要每人本地安装一次：`ln -sf ../../references/scripts/pre-commit.sh .git/hooks/pre-commit`。可用 `--no-verify` 临时绕过（不推荐）。
- **跨 skill 共享文件一致性检查**（跨 skill，治理动作）：`references/scripts/check_docs_consistency.sh` 验证 AGENTS.md §「Shared files」表声明的"哪些 skill 读哪些共享文件"与实际 grep 结果是否一致，捕获两类 drift：(1) **Over-claim**（AGENTS.md 说 X 读 Y，但 X 的 SKILL.md / references/ 实际不引用 Y）；(2) **Under-claim**（X 实际引用 Y，但 AGENTS.md 没把 X 列入读者）。脚本里的 `SHARED_FILES` 数组与 AGENTS.md 共享文件表是双 source of truth，**改一边必须同步另一边**。同时验证 `$LANLNK_BASE/{incoming,raw,materials,out/proposals,out/bidding,config/ontology}` 等关键目录存在。**适用边界（2026-07-19 用户决策）**：本脚本只治理 skill 仓库自身和 `/opt/code/docs` 共享基础设施，不审 mi / langchat 等独立软件仓库。
...
content-operations ─ 独立（lnkwebsite 内容研究→生成→校验→CMS 草稿导入）
